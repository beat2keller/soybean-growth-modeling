...
colnames(genotype_selection) = c('genotype', 'prec_int','prec','rad_int','rad', 'rad_low','prec_low','scal','asymp','rad_int_2','asym_int')
...
for (i in 1:length(unique(df$genotype.id))) {
  ...
  genotype_selection[i, 5] = ifelse(rad[i] < quantile(rad)[1], 1, 0) # 
  genotype_selection[i, 6] = ifelse(rad[i] < quantile(rad)[2], 1, 0) #  increase the weight of rad
  ...
  genotype_selection[i, 8] = ifelse(rad_intervals[i] < quantile(rad_intervals)[3], 1,0) #  increase the weight of rad_interval
...
p$Size <- 1
p$Size[p$genotype.id%in%candidate_genotypes] <- 3
...
overview_means$Scale[overview_means$Interaction=="precipitation_14"] <- "Gen:Pre"
overview_means$Scale[overview_means$Interaction=="radiation_14"] <- "Gen:R"
overview_means$Scale[overview_means$Interaction=="photothermal_14"] <- "Gen:P"
